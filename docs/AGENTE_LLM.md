...
| `--attempts` | inteiro ≥ 1, ex.: `1` (nunca repete), `3`, `5` | 3 | quantas vezes chamar quando a resposta chega mas não serve: JSON inválido, estratégia acima de 260 caracteres, HTTP 429 ou 5xx |
...
```
      ...
      │  "estrategia": bullets, máx. 260 caracteres, é a âncora │
      └─────────────────────────────────────────────────────────┘
                              │
                              v
      ┌─ POR DIA ───────────────────────────────────────────────┐
      │  código monta:  estratégia (bullets, 260 caracteres)     │
...
estratégia acima de 260 caracteres, HTTP 429/5xx.
...
| sem apodrecimento | validade + `LIMPAR` | planta pronta apodrece |
| sem fertilizante | `FERTILIZAR`, `COMPRAR fertilizante n` | regras deste jogo |
| `PLANTAR <cultivo> (TUDO \| LIMITE n)` | um quantificador só: `LIMITE` opcional em todo verbo, `TUDO` (ou nada) no campo, número obrigatório em `COMPRAR` | `PLANTAR trigo 5`, `COLHER TUDO` e `LIMPAR TUDO` eram os erros de gramática mais comuns nas runs |
